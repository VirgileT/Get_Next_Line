...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/15 19:25:10 by vtestut           #+#    #+#             */
/*   Updated: 2022/12/21 23:30:13 by vtestut          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	buffer[fd] = ft_read_and_add(fd, buffer[fd]);
	if (!buffer[fd])
		return (NULL);
	line = ft_get_line(buffer[fd]);
	buffer[fd] = ft_clear_buffer(buffer[fd]);
	return (line);
}
